'#TEMPLATE DDT Dialog with a ListBox listing a directory
...
FUNCTION wWinMain (BYVAL hInstance AS HINSTANCE, _
   ...
   ' // Create a new dialog
   DIM hDlg AS HWND = DialogNewPixels(0, "DDT - DlgDirList", 0, 0, 320, 360, WS_OVERLAPPEDWINDOW)
   ' // Center the window
   DialogCenter(hDlg)

   ' // Add a label to display the path
   ControlAddLabel, hDlg, IDC_PATH, "",  8, 8, 300, 23
   ' // Anchor the label
   ControlAnchor hDlg, IDC_PATH, AFX_ANCHOR_WIDTH

   ' // Add a listbox
   ControlAddListBox, hDlg, IDC_LISTBOX, "", 8, 30, 300, 280
...
'  ' // Anchor the controls
   ...
   DlgDirListW(hDlg, AfxGetExePath, IDC_LISTBOX, IDC_PATH, 0)
   ' // Select the first item
   ListBoxSelect hDlg, IDC_LISTBOX, 1

   ' // Add a cancel button
   ControlAddButton, hDlg, IDCANCEL, "&Cancel", 233, 325, 75, 23
   ' // Anchor the button
